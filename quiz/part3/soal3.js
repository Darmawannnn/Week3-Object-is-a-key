...
function countProfit(shoppers) {
    ...
    for (let i = 0; i < listBarang.length; i++) {
        const product = listBarang[i];
        const pembeli = [];
        let sisaStock = product[2];
        let totalProfit = 0;

        for (let j = 0; j < shoppers.length; j++) {
        const shopper = shoppers[j];

        if (shopper.product === product[0] && shopper.amount <= sisaStock) {
            pembeli.push(shopper.name);
            sisaStock -= shopper.amount;
            totalProfit += shopper.amount * product[1];
        }
        }
